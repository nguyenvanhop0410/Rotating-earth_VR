...
export function drawThreeGeo({ json, radius, materialOptions, materalOptions }) {
  const resolvedOptions = materialOptions ?? materalOptions ?? {};
  ...
  container.userData.onResize = (w, h) => {
    for (let i = 0; i < container.children.length; i++) {
      container.children[i].userData.onResize?.(w, h);
    }
  };
  ...
  for (let geom_num = 0; geom_num < json_geom.length; geom_num++) {
    if (json_geom[geom_num].type === 'Point') {
      ...
      drawParticle(x_values[0], y_values[0], z_values[0], resolvedOptions);
      ...
      drawLine(x_values, y_values, z_values, resolvedOptions);
  ...
  function drawLine(x_values, y_values, z_values, options) {
    ...
    const rate = options.animateDash ? (options.dashRate ?? Math.random() * 0.0002) : 0;
    line.userData.update = (t) => {
      if (lineMaterial.dashed && rate) {
        lineMaterial.dashOffset = t * rate;
      }
    };
    line.userData.onResize = (w, h) => {
